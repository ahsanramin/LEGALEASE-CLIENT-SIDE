...
import { Users, Briefcase, Clock, CheckCircle, FileText, Pencil, Eye, Calendar, DollarSign, ArrowUpRight, ArrowRight } from 'lucide-react';
...
export default function LawyerDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [profileRes, hiringRes] = await Promise.all([
          axios.get('/lawyers/me/profile'),
          axios.get('/hiring/lawyer')
        ]);
        ...
        const totalHires = hiringData.length;
        const pendingRequests = hiringData.filter(h => h.status === 'pending').length;
        const activeClients = new Set(hiringData.map(h => h.userId?._id)).size;

        let strength = 0;
        if (profileData.image) strength += 20;
        if (profileData.name && profileData.name.length > 2) strength += 15;
        if (profileData.specialization && profileData.specialization.length > 2) strength += 15;
        if (profileData.bio && profileData.bio.length > 30) strength += 20;
        if (profileData.fee && parseFloat(profileData.fee) > 0) strength += 15;
        if (profileData.location && profileData.location.length > 2) strength += 15;

        setStats({ totalHires, pendingRequests, activeClients, profileStrength: Math.min(100, strength) });
    ...
    <div className="space-y-8">
      ...
      >
        {[
          ...
          { label: 'Active Clients', value: stats.activeClients, icon: Users, color: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800' },
          { label: 'Pending Requests', value: stats.pendingRequests, icon: Clock, color: 'bg-yellow-50 text-yellow-600 dark:bg-yellow-900/20 dark:text-yellow-400 border-yellow-200 dark:border-yellow-800' },
          { label: 'Profile Strength', value: `${stats.profileStrength}%`, icon: CheckCircle, color: 'bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400 border-purple-200 dark:border-purple-800' }
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-white/80 dark:bg-[#1a1c23]/80 backdrop-blur-sm border border-gray-100/50 dark:border-gray-700/50 rounded-3xl shadow-xl overflow-hidden">
          <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
              <FileText size={20} className="text-indigo-600" /> Recent Hiring Requests
            </h3>
            <button onClick={() => router.push('/dashboard/lawyer/hiring-history')} className="text-sm text-indigo-600 hover:text-indigo-700 font-medium flex items-center gap-1">
          ...
          <div className="p-6">
            {hires.length === 0 ? (
              <div className="text-center py-8 text-gray-500 dark:text-gray-400 flex flex-col items-center gap-2">
                ...
                <p className="text-sm">When clients hire you, they will appear here.</p>
              </div>
            ) : (
              <div className="space-y-4">
                {hires.slice(0, 4).map((hire) => (
                  <div key={hire._id} className="group flex items-center justify-between p-4 bg-gray-50/50 dark:bg-[#0f1115]/50 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 hover:bg-white dark:hover:bg-[#1a1c23] transition-all">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-800 dark:text-gray-100">{hire.userId?.name || 'Anonymous Client'}</p>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                        hire.status === 'pending' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800' :
                        hire.status === 'accepted' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800' :
                      ...
                      {hire.status === 'pending' && (
                        <button onClick={() => router.push('/dashboard/lawyer/hiring-history')} className="text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors">
        ...
        <motion.div variants={itemVariants} className="space-y-6">
          <div className="bg-white/80 dark:bg-[#1a1c23]/80 backdrop-blur-sm border border-gray-100/50 dark:border-gray-700/50 rounded-3xl shadow-xl p-6 overflow-hidden relative">
            ...
            <div className="space-y-3">
              <button onClick={() => router.push('/dashboard/lawyer/manage-legal-profile')} className="w-full flex items-center justify-between p-4 bg-gray-50/70 dark:bg-[#0f1115]/70 rounded-2xl hover:bg-gray-100 dark:hover:bg-[#1a1c23] transition-colors border border-gray-200/50 dark:border-gray-700/50 group">
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => router.push('/dashboard/lawyer/hiring-history')} className="w-full flex items-center justify-between p-4 bg-gray-50/70 dark:bg-[#0f1115]/70 rounded-2xl hover:bg-gray-100 dark:hover:bg-[#1a1c23] transition-colors border border-gray-200/50 dark:border-gray-700/50 group">
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => router.push(`/lawyer/${profile?._id}`)} className="w-full flex items-center justify-between p-4 bg-gray-50/70 dark:bg-[#0f1115]/70 rounded-2xl hover:bg-gray-100 dark:hover:bg-[#1a1c23] transition-colors border border-gray-200/50 dark:border-gray-700/50 group">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-lg">
                    <Eye size={18} className="text-purple-600 dark:text-purple-400" />
                  </div>
                  <span className="font-medium text-gray-700 dark:text-gray-300">View Public Profile</span>
                </div>
          ...
          <div className="bg-white/80 dark:bg-[#1a1c23]/80 backdrop-blur-sm border border-gray-100/50 dark:border-gray-700/50 rounded-3xl shadow-xl p-6">
            <h3 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3 flex items-center gap-2">
              <CheckCircle size={16} /> Profile Strength
            </h3>
            ...
            <div className="flex items-center justify-between mt-3">
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {stats.profileStrength < 50 ? 'Start completing your profile!' : 
                 stats.profileStrength < 80 ? 'You are almost there!' : 
                 'Profile is fully completed!'}
              </span>
              <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400">{stats.profileStrength}%</span>
            </div>
          </div>
        </motion.div>
      </motion.div>
